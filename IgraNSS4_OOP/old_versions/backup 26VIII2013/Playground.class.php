...
// var_dump($r_ar);
  ...
  public function pawnsData() {
    ...
    foreach ($pos as $cell) {
      if (!empty($cell[4])) {
        $p_rend[0]= $c;
        $p_rend[1]= $this->links[$c]->render('numbers');
        $p_rend[2]= $cell[4][0]->getColor();
...
/*  public function renderAllPawns() {
    ...
    foreach ($pos as $cell) {
      if (!empty($cell[4])) {
        $p_rend[0]= htmlentities($c); // htmlentities()
        $p_rend[0]= stripslashes($p_rend[0]);
        $p_rend[1]= htmlentities($this->render($this->links[$c])); // htmlentities()
        $p_rend[1]= stripslashes($p_rend[1]);
        $all_pawns[]=$p_rend;
      }
      $c++;
