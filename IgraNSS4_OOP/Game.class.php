...
// private $exit_data;
  ...
  public function __construct($players_count, $pl1_color) {
    if (isset($players_count) && isset($pl1_color)) {
      ...
      $this->exception='';
      $this->turn=-1;
      $this->pawns_data=array();
      $this->turn_data=array('', '', '', '');
  ...
  private function calcPawnsData() {
    // [0] - position number; // [1] - pawns color; // [2] - pawns numbers;
      $this->pawns_data=$this->plgr->pawnsData();
  }

  public function getPawnsData() {
    return $this->pawns_data;
  }

  public function getTurnData() {
  /*turn_data[0][0] - player name
    turn_data[0][1] - player color
    turn_data[1] - dice->value()
    turn_data[2] - radio number check
    turn_data[3] - turn number
    turn_data[4] - exception
     */
  ...
  public function tableExit($return=''){
    $this->nextMove();
  ...
  private function gameControl($return='') {
    // echo 'count(this->players): '.count($this->players).'<br />';
    // echo 'move: '.$this->move.'<br />';
    $this->turnCount();
    $this->turn_data[0]=array();
    $this->turn_data[0][0]= $this->players[$this->move]->getName();
    $this->turn_data[0][1]= $this->players[$this->move]->getColor();
    if ($return!='game-play') {
    echo '<table border="1" background="pics/back_command.jpg"><tr><td>';
    echo '<p id="pl_name">'.$this->players[$this->move]->render().'</p>';    
    echo '</td><td id="dice" width="51" height="48" valign="top">'.$this->dice->render();
    ...
    if ($return!='game-play') {
    echo <<<_END
    Move pawn No<span id="radio">
    <input type="radio" id="r1" name="pawn_num" value="1" $checked[0]> 1 |
    <input type="radio" id="r2" name="pawn_num" value="2" $checked[1]> 2 |
    <input type="radio" id="r3" name="pawn_num" value="3" $checked[2]> 3 |
    <input type="radio" id="r4" name="pawn_num" value="4" $checked[3]> 4 </span>
...
_END;
    ...
    echo '<input type="button" id="play" value="Следващ ход">';
    echo '</form>';
    echo '</td></tr></table>';
    echo '<p><a href="index1.php">Нова игра</a></p>';
    echo '<span class="ex">'.$this->exception.'</span>';
    }
    $this->calcPawnsData();
    $this->turn_data[1]=$this->dice->value();
    $this->turn_data[3]= $this->turn;
    $this->turn_data[4]=addslashes($this->exception);
  }
 ...
 // Check if the game is over.
    ...
    if ($cp== count($this->players)) {
      return 'Играта приключи.<br /><a href="index1.php">Играйте отново</a>';
    }
// Check if this player is finished.
    ...
    if ($c==4) {
      return 'Вие сте приключили играта. Поздравления!
              <br />Следващият е на ход.';
    }

    if ($pawn_pos_num>$gfinal[$this->move] && $dice!=6) {
      return 'Нуждаете се от 6-ца.
              <br />Следващият е на ход.';
    }
    
    if ($pawn_pos_num==$gfinal[$this->move]) {
      return 'Тази пионка приключи своя ход!
              <br />Следващият е на ход.';
    }  
    if ($pawn_pos_num<$gfinal[$this->move] &&
      $pawn_pos_num+$dice>$gfinal[$this->move]) {
      return 'Нуждаете се от точен зар!
              <br />Следващият е на ход.';
    }
